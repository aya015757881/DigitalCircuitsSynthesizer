...
void extract()
{
	...
	for(i=0;i<10000;i++)
		a[i]=0;
	printf("Please key in the name of the Verilog source file that you want to synthesize.\nFile name: ");
 s1:gets(file);
	strcpy(filetxt,file);
	strcat(filetxt,".txt");
	if((fp=fopen(filetxt,"r"))==NULL)
	{
		printf("\nFile %s does not exist, key in the right file name:\n",filetxt);
		goto s1;
	}
	for(i=0;(a[i]=fgetc(fp))!=EOF;i++);
	a[i]=0;
	fclose(fp);
	printf("\nNetlist file of %s is being synthesized, please wait...\n",filetxt);
}
void find_var()
{
...
void display()
{
	...
	fclose(fp);
	printf("\nNetlist file of %s %s has been generated in the directory where Synthesizer.exe is, check it.\n\n\n\n",filetxt,filectt);
}
void clear()
{
	in=&in_head;
...
void ge_ee(int idx)
{
	for(first(ee_name);a[n];next(ee_name))
		if(a[n+ee_name_len]=='~'||a[n+ee_name_len]>='a'&&a[n+ee_name_len]<='z') gen_ee1(idx);
		else gen_ee2(idx);
}
void sm_ee()
{
...
int check_sm()
{
	...
	strcat(c,"<=");
	for(first(c);a[n];next(c))
		if(n>=sm.start&&n<=sm.end) return 1;
	if(check_num(out->p->name))
	{
		for(i=0;out->p->name[i]!='[';i++)
			c[i]=out->p->name[i];
...
void get_cp_rst()
{
	...
	for(out=&out_head;out->p!=NULL;out=out->p)
	{
		...
		for(gc=&gc_head;gc->p!=NULL;gc=gc->p)
			if(!strcmp(gc->p->in_p->name,clock->name)&&addr>=gc->p->start&&addr<=gc->p->end)
			{
...
void add_gcv()
{
	for(gc=&gc_head;gc->p!=NULL;gc=gc->p)
	{
		...
		else if(n<gc->p->start||n>gc->p->end) continue;
		else
		{
			rv->p=calloc(1,sizeof(RV));
			rv->p->in_p=gc->p->in_p;
...
void pickvar(int idx,int start,int end)
{
	...
	{
		(ive1+idx)->num=a;
		if(end<n-1) pickvar(idx+1,a+1,end+1);
		else posneg();
		if(allcov) break;
	}
}
...
{
	for(al=&al_head;al->p!=NULL;al=al->p)
		if(gc->p->start>=al->p->start&&gc->p->end<=al->p->end&&gc->p->st==al_state()) return 1;
	return 0;
}
...
int check_and_num()
{
	...
	if(i1==j1&&i2==j2&&i3==j3&&i4==j4) return 1;
	else return 0;
}
int check_and_var()
{
...
int cov()
{
	int i;
	for(i=0;i<count;i++)
		if((tt+i)->val&&!(tt+i)->idx) return 0;
	return 1;
}
int check_in_st()
{
	for(ive=&imp->p->ive_head;ive->p!=NULL;ive=ive->p)
		if(!check_reg()&&ive->p->st&&in_num()==iu->p->n) return 1;
	return 0;
}
int check_reg_st()
{
	for(ive=&imp->p->ive_head;ive->p!=NULL;ive=ive->p)
		if(check_reg()&&ive->p->st&&reg_num()==ru->p->n) return 1;
	return 0;
}
int check_regn_st()
{
	for(ive=&imp->p->ive_head;ive->p!=NULL;ive=ive->p)
		if(check_reg()&&!ive->p->st&&reg_num()==rnu->p->n) return 1;
	return 0;
}
int check_not_st()
{
	for(ive=&imp->p->ive_head;ive->p!=NULL;ive=ive->p)
		if(!check_reg()&&!ive->p->st&&not_num()==nu->p->n) return 1;
	return 0;
}
int match(int val)
{
	...
	get_bn(val);
	for(i=0;i<qua;i++)
		if(*(p+(ive1+i)->num)!=(ive1+i)->st) return 0;
	return 1;
}
int check_reg()
{
